...
class AuthorizationModel extends BaseModel
{
    ...
    /**
     * @var array<int, array{columnName: string, columnAlias: string, validations?: array{required: bool}}> List of all columns
     */
    ...
    {
        $this->columns = array(
            array('columnName' => $this->primaryKey, 'validations' => array('required' => false)),
            array('columnName' => 'userrole'),
            array('columnName' => 'useridno'),
            array('columnName' => 'username', 'validations' => array('required' => true)),
            array('columnName' => 'password', 'validations' => array('required' => true, 'inputtype' => 'password')),
            array('columnName' => 'isactive', 'validations' => array('required' => true, 'in_array' => array('T', 'F'))),
            array('columnName' => 'passchange', 'validations' => array('required' => true)),
    ...
    public function signin(string $username, string $password): array
    {
        try {
            ...
            if (!$check['status']) {
                ...
                throw new Exception($this->EmptyOrMoreError("Authorization Detail", $check['resultsCount']));
            } else {
                // return $check;
